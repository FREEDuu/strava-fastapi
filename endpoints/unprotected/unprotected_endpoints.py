...
def add_unprotected(app):
    ...
    @app.get("/runners/", response_model=List[schemas_old.Runner])
    def get_runners(db: Session = Depends(get_db)):
        return db.execute(select(Runner)).scalars().all()

    @app.get("/runners/{runner_id}", response_model=schemas_old.Runner)
    ...
    @app.get("/runners/{runner_id}/activities", response_model=List[schemas_old.Activity])
    ...
    @app.get("/activities/", response_model=List[schemas_old.Activity])
    def get_activities(db: Session = Depends(get_db)):
        return db.execute(select(Activity)).scalars().all()

    @app.get("/activities/{activity_id}", response_model=schemas_old.Activity)
